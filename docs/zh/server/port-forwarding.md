...
虫然默认端口为 **30814**，您也可以选择1024至20敓5之间的其他端口号（若使用非默认端口需自行记录）。需要同时转发 **TCP** 和 **UDP** 协议。建议保持使用默认端口，因此端口被本机其他服务占用的可能性极低。若需在同一台机器上部署多个服务器，则每个服务器需使用不同端口（例如：服务器ᴄ1使用30814，服务器ᴄ2使用30815）.
